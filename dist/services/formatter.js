...
// ---------------------------------------------------------------------------
export function formatJob(job, index) {
    const lines = [];
    ...
    if (job.workplaceTypes?.length)
        lines.push(`   🏠 Workplace: ${job.workplaceTypes.join(", ")}`);
    if (job.salary)
    ...
    if (job.detailsPageUrl)
        lines.push(`   🔗 View Job: ${job.detailsPageUrl}`);
    return lines.join("\n");
}
// ---------------------------------------------------------------------------
...
export function formatJobDetail(job) {
    ...
    if (job.postedDate)
        lines.push(`**Posted:** ${new Date(job.postedDate).toLocaleDateString()}`);
    if (job.detailsPageUrl)
        lines.push(`**Job URL:** ${job.detailsPageUrl}`);
    if (job.applyUrl)
...
// ---------------------------------------------------------------------------
export function formatSearchResults(result, searchLabel) {
